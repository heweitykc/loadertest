...
public interface TLoggerListener
{
    void log(string msg);
};
...
public class TLogger : TStaticHelperBase<TLogger> {
    ...
    private List<TLoggerListener> _listeners;
    ...
    protected override void onInit()
    {
        ...
        _listeners = new List<TLoggerListener>();
    }
    ...
    public void addLogListener(TLoggerListener listener)
    {
        if (!hasInited)
            return;
    ...
    public void log(string msg, LOGGER_CHANNEL channel, bool simpleMode)
    {
        ...
        if (simpleMode)
        {
            ...
            outputMsg = 
                "(at" + DateTime.Now.ToString() + ")" + "[" + channel.ToString() + "]" + msg;
        }
        foreach(TLoggerListener listener in _listeners)
